<script lang="ts">
	...
	$effect(() => {
		...
		untrack(() => {
			showShare =
				containerPadding + detailsWidth + gapPadding + navigateWidth + gapPadding + shareWidth <=
				containerWidth;
...
<div class="flex px-4 gap-1.5 absolute bottom-4 w-full" bind:clientWidth={containerWidth}>
	...
	<div bind:clientWidth={navigateWidth}>
		<Button size="default" variant="outline" tag="a" href={getMapsUrl(lat, lon)} target="_blank">
			<Navigation size="18" />
			{#if showNavigate}
				<span>
					{m.popup_navigate()}
	...
	{#if canNativeShare() || hasClipboardWrite()}
		<div bind:clientWidth={shareWidth}>
			<Button variant="outline" tag="button" onclick={shareUrl}>
				<Share2 size="18" />
				{#if showShare}
					<span>
						{m.popup_share()}
