...
{#if page.url.pathname.startsWith("/coverage")}
	...
	>
		<Drawer.Portal>
			<Drawer.Content
				class="duration-150! rounded-t-xl fixed flex flex-col bottom-0 z-10 px-2 py-2 w-full h-full border border-t-border bg-card/60 backdrop-blur-sm focus:outline-none pb-[env(safe-area-inset-bottom)]"
			>
